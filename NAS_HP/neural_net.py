...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, image_size, width_multip=1,  num_classes=10):
        ...
        self.in_planes = 8*int((64 * width_multip)//8)

        self.conv1 = nn.Conv2d(3, self.in_planes, kernel_size=3,
                               stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(self.in_planes)
        self.layer1 = self._make_layer(block, self.in_planes, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 8*int((128 * width_multip)//8), num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 8*int((256 * width_multip)//8), num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, 8*int((512 * width_multip)//8), num_blocks[3], stride=2)
